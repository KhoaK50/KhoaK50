...
# Use a more robust regex that catches the entire fetch chain
html = re.sub(r'\.then\(function\(r\)\{ return r\.json\(\); \}\)\s*\.then\(function\(data\)\{\s*sendBtn\.innerHTML =.*?\}\)\s*\.catch\(function\(err\)\{', replacement + '\n      .catch(function(err){', html, flags=re.DOTALL)
