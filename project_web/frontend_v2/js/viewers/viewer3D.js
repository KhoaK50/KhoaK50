...
(function () {
    ...
    Vec3D.addAxisLabelsDynamic = function () {
        ...
        const putLabel = (axis, tMath) => {
            ...
            inner.style.color = axis === "x" ? (App.getCSS?.("--axis-x") || "red") : axis === "y" ? (App.getCSS?.("--axis-y") || "green") : (App.getCSS?.("--axis-z") || "blue");
        ...
        const addLetter = (txt, axis, position) => {
            ...
            el.textContent = txt;
            el.style.color = axis === "x" ? (App.getCSS?.("--axis-x") || "red") : axis === "y" ? (App.getCSS?.("--axis-y") || "green") : (App.getCSS?.("--axis-z") || "blue");
    ...
    Vec3D.draw3DAllVectors = function (opts = { frame: false }) {
        ...
        for (const it of list) {
            ...
            const proj = Vec3D.buildProjectionGroupZUp([tipLocal.x, tipLocal.y, tipLocal.z], App.getCSS?.("--axis") || "#888", aItem * 0.9);
            const el = document.createElement("div");
        ...
        if (opts.frame) {
            ...
            if (!hasVec) {
                ...
                const longest = Math.max(...App.vectorList.map((it) => {
        ...
        if (App.currentVector) {
            const txt = App.formatTip ? App.formatTip(App.currentVector) : `[${App.currentVector.join(",")}]`;
    ...
    Vec3D.refreshAngleTheme = function () {
        const g = App.currentAngleVisual3D;
        if (!g) return;
        const color = new THREE.Color(App.getCSS?.("--angle-fill") || "rgba(255,200,0,0.3)");
    ...
    Vec3D.drawAngleArc3D = function (v1, v2, rad, deg) {
        ...
        const curve = new THREE.EllipseCurve(
            0, 0,            // ax, aY
            displayRadius, displayRadius, // xRadius, yRadius
            0, angleVal,     // startAngle, endAngle
            false,           // clockwise
            0                // rotation
        );
        ...
        const startVecLocal = new THREE.Vector3(1, 0, 0).applyQuaternion(quaternion);
        ...
        const midDir = new THREE.Vector3(Math.cos(midAngle), Math.sin(midAngle), 0).applyQuaternion(group.quaternion);
