...
(function () {
  ...
  // Hiển thị thanh trạng thái Chế độ Khảo sát
  App.showReplayHUD = function() {
      let hud = document.getElementById("replayHUD");
      if (!hud) {
          hud = document.createElement("div");
          hud.id = "replayHUD";
          hud.style.position = "fixed";
          hud.style.top = "100px"; // Cố tình hạ thấp xuống để tránh bị topbar che mất
          hud.style.left = "50%";
          hud.style.transform = "translateX(-50%)";
          hud.style.zIndex = "9999999";
          hud.style.background = "var(--bg-surface)";
          hud.style.padding = "10px 20px";
          hud.style.borderRadius = "30px";
          hud.style.boxShadow = "0 8px 32px rgba(0, 0, 0, 0.15)";
          hud.style.border = "1px solid var(--border-subtle)";
          hud.style.display = "flex";
          hud.style.alignItems = "center";
          hud.style.gap = "15px";
          hud.style.color = "var(--text-main)";
  ...
  // Hàm dọn dẹp các bóng ma sau khi chiếu lại hoàn tất (tự động gọi)
  App.cleanupReplayGhosts = function() {
      const hud = document.getElementById("replayHUD");
      if (hud) hud.remove();

      if (!window.App || !window.App._preReplayVectorList) return;
      
      // Khôi phục 100% thực tại từ bản backup
      window.App.vectorList = JSON.parse(JSON.stringify(window.App._preReplayVectorList));
      window.App._preReplayVectorList = null;
      
      if (window.App.renderVectorList) window.App.renderVectorList();
      if (window.App.refreshCalcVectorOptions) window.App.refreshCalcVectorOptions();
      if (window.App.redrawAll) window.App.redrawAll({ frame: false }); // Vẽ lại để cập nhật 3D canvas
  };
  ...
  function toggleComputeBtn(disable) {
      ...
      if (!disable && window.App) {
          if (App.unlockInteraction) App.unlockInteraction();
          // Xóa dòng tự động dọn dẹp bóng ma
      }
  }
  ...
  function toggleComputeBtn(disable) {
      ...
      if (!disable && window.App && App.unlockInteraction) {
          App.unlockInteraction();
      }
  }
  ...
  App.animateOperation = function (op, ids, resValOrId) {
    if (op === "add") {
      ...
      if (App.lockInteraction) App.lockInteraction(); 
