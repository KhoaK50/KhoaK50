...
(function () {
    ...
    const Easing = {
        ...
        elasticOut: function (t) {
            if (t === 0) return 0;
            if (t === 1) return 1;
            return Math.pow(2, -3 * t) * Math.sin((t * 15 - 0.75) * (2 * Math.PI) / 3) + 1;
        }
    };
    ...
    function fadeUnrelatedVectors(keepIds, targetAlpha, callback) {
        ...
        const startAlpha = targetVectors[0].alpha !== undefined ? targetVectors[0].alpha : 1;

        runTween(CONFIG.DURATION.PRE_FADE, Easing.linear, function (v) {
    ...
    function buildFlashlight3D() {
        ...
        const matBody = new THREE.MeshPhongMaterial({ color: CONFIG.COLOR.LAMP_BODY, shininess: 40 });
        const matRing = new THREE.MeshPhongMaterial({ color: CONFIG.COLOR.LAMP_RING, shininess: 80 });
        const matGlass = new THREE.MeshBasicMaterial({ color: CONFIG.COLOR.LAMP_GLASS });
    ...
    function runAnimProj3D(v1Id, v2Id, resId) {
        ...
        fadeUnrelatedVectors([v1Id, v2Id, resId], 0.1, function () {
            ...
            const resTip = resG ? resG.userData.tipLocal.clone() : new THREE.Vector3(res.vec[0], res.vec[1], res.vec[2] || 0).multiplyScalar(Vec3D.S3D.unitsPerWorld);
            const normal = new THREE.Vector3().subVectors(v1Tip, resTip);
            let dist = normal.length();
            if (dist < 0.01) { normal.set(0, 0, 1); dist = 3; }

            const lampPos = v1Tip.clone().add(normal.normalize().multiplyScalar(CONFIG.GEOMETRY.LAMP_OFFSET_3D));
            ...
            // 3. Animation
            runTween(CONFIG.DURATION.POPUP, Easing.easeOutBack, function (v) {
                flashlight.scale.setScalar(v);
            }, function () {
                const lit = flashlight.userData.light;
                runTween(CONFIG.DURATION.BEAM_ON, Easing.linear, function (v) {
                    ...
                    // Bóng mọc ra
                    runTween(CONFIG.DURATION.SHADOW_GROW, Easing.easeInOutCubic, function (v) {

                    }, function () {
                        // Kết thúc: Tắt đèn, Bóng mờ, Thật hiện
                        runTween(CONFIG.DURATION.CLEANUP, Easing.linear, function (v) {
                            ...
                            res.alpha = 1;
                            if (resG) { resG.visible = true; setOpacity3D(resG, 1); }
    ...
    function runAnimAdd3D(v1Id, v2Id, resId) {
        ...
        fadeUnrelatedVectors([v1Id, v2Id, resId], 0.1, function () {
            ...
            runTween(CONFIG.DURATION.SLIDE, Easing.easeInOutCubic, function (v) {
    ...
    function runAnimAdd2D(v1Id, v2Id, resId) {
        ...
        fadeUnrelatedVectors([v1Id, v2Id, resId], 0.1, function () {
            const g = { vec: v2.vec, colorCss: "#ffaa00", alpha: 0.6, isGhost: true, offset: [0, 0] };
            App.tempGhosts = [g];
            res.alpha = 0;
            const ex = v1.vec[0];
            const ey = v1.vec[1];
