    ...
    function init() {
        ...
        if (btnCalc) {
            btnCalc.addEventListener("click", () => {
                ...
                const checkedInputs = Array.from(checkContainer.querySelectorAll('input[type="checkbox"]:checked'));
                const basisObjs = checkedInputs.map(inp => {
                ...
                if (basisObjs.length !== n) {
                    alert(`Lưu ý: Để tìm tọa độ trong R^${n}, hệ cơ sở cần có đúng ${n} vector.\n(Bạn đang chọn ${basisObjs.length} vector)`);
