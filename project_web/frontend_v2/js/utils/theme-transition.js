...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (themeToggle) {
    ...
    // sync checkbox
    try { themeToggle.checked = ThemeManager.isDarkMode(); } catch (e) {}

    themeToggle.addEventListener('change', () => {
      console.debug('[theme-transition] toggle change');
      const isDark = (typeof ThemeManager !== 'undefined') ? ThemeManager.toggle() : false;
      try { ThemeManager.applyTheme(isDark); } catch (e) { console.error('[theme-transition] applyTheme error', e); }
  ...
  window.addEventListener('storage', (e) => {
    if (e.key && typeof ThemeManager !== 'undefined' && e.key === ThemeManager.STORAGE_KEY) {
      const isDark = ThemeManager.isDarkMode();
      if (themeToggle) themeToggle.checked = isDark;
      try { ThemeManager.applyTheme(isDark); } catch (err) {}
      playTransition(isDark);
    }
  });
});
